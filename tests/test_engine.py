"""
Integration tests for the screen recorder capture and muxing engine.
"""
...
class TestScreenRecorderEngine(unittest.TestCase):
    ...
    def test_02_headless_recording_cycle(self):
        """Test a 3-second recording cycle and check file validity."""
        ...
        print("\n[Test] Starting 3-second recording cycle...")
        started = controller.start_recording(region=region)
        self.assertTrue(started, "Recording controller must start cleanly")
        ...
        self.assertGreater(file_size, 1000, "Output file must contain encoded video bytes")
        print(f"[Test] Recording successfully created: {output_file} ({file_size} bytes)")
